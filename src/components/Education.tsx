...
const Education: React.FC = () => {
  return (
    <section id="education" className="py-20 relative bg-white/10">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <p className="text-accent font-medium tracking-wide mb-2 uppercase text-sm">Academic Journey</p>
          <h2 className="text-4xl md:text-5xl font-bold font-heading text-gray-900">Education</h2>
        </div>

        <div className="space-y-8">
          {educationData.map((edu, index) => (
            <div key={index} className="glass p-8 rounded-2xl relative border-l-4 border-primary hover:bg-black/5 transition-colors duration-300 bg-white/50">
              ...
              <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
                ...
                <div className="text-right md:text-right">
                  <span className="block text-sm font-semibold text-gray-600 bg-black/5 px-4 py-1.5 rounded-full inline-block mb-1">
                    {edu.period}
                  </span>
              ...
              <ul className="space-y-3 mb-4">
                {edu.details.map((detail: string, idx: number) => (
                  <li key={idx} className="text-gray-600 flex items-start gap-2.5">
